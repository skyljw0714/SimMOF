...
- **Pore diameters:**  
  ...
  probe_diameter : null
  num_samples : null
  you must not use `probe_diameter` and `num_samples` if you use `-ha -res`

- **Accessible surface area:**  
  Calculates the surface area accessible to a spherical probe. 
  command : -ha -sa
  probe_diameter : default is 1.2(H2), depands on the guest molecule
...
- You must use the default value for `probe_diameter` and `num_samples` if no specific value is provided.
